...
def load_model():
    model = (
        AutoModelForCausalLM.from_pretrained("wuyue2247/Recipe_Q-A_Assistant", trust_remote_code=True)
        .to(torch.bfloat16)
        .cuda()
    )
    tokenizer = AutoTokenizer.from_pretrained("wuyue2247/Recipe_Q-A_Assistant", trust_remote_code=True)
    ...
    if prompt := st.chat_input("What is up?"):
        ...
        if not contains_keywords:
            with st.chat_message("robot", avatar=robot_avator):
                st.markdown("我是你的智能菜谱问答小助手，我什么菜都会做，包括黑暗料理，您可以问我什么菜怎么做———比如酸菜鱼怎么做？，我会告诉你具体的做法。")
            # Add robot response to chat history
            st.session_state.messages.append({"role": "robot", "content": "我是你的智能菜谱问答小助手，我什么菜都会做，包括黑暗料理，您可以问我什么菜怎么做———比如酸菜鱼怎么做？，我会告诉你具体的做法。", "avatar": robot_avator})
        else:
